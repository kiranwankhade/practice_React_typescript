...
const SignUp  = () => {
  ...
  const isPhoneValid = (/^\d{10}$/.test(phoneNumber) && phoneNumber.length) === 10; //Check Phone number is in Number format or not And Have 10 digit only
  const isEnabled = name && isEmailValid && isPhoneValid;

  const navigate = useNavigate();

  // const handleSignUp = async () => {
  //   const userData = { name, email, phoneNumber, referralCode };
  //   try {
  //     const response = await axios.post('https://agreeable-calf-coat.cyclic.cloud/user/register', userData);
  //     if (response.data) {
  ...
  const signUpMutation = useMutation(
    async (userData: { name: string; email: string; phoneNumber: string; referralCode: string }) => {
      const response = await axios.post('https://agreeable-calf-coat.cyclic.cloud/user/register', userData);
      if (response.data) {
    ...
    {
      onSuccess: (data) => {
        // Handle success, e.g., saving to local storage and navigating
