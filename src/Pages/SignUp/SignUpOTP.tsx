import {
  ...
  const verifyOtpMutation = useMutation(
    async (userData: { name: string; email: string; phoneNumber: string; otp: string }) => {
      const response = await axios.post('https://agreeable-calf-coat.cyclic.cloud/user/verify-register', userData);
      if (response.data) {
        return response.data;
      }
          ...
          <Box mt={2} w={"100%"} p={"1rem"}>
            <FormControl w={"100%"} mt={1}>
              <FormLabel
                ...
                fontWeight={600}>
                Enter OTP{" "}
                {newUsers.message === "Verification code sent successfully to email."
                  ? `From ${newUsers.email}`
